...
class _HomeState extends State<Home> {
  ...
  try {
    ...
    if (currentResponse.statusCode == 200 && forecastResponse.statusCode == 200) {
      ...
      setState(() {
        ...
        hourlyForecast = forecastData['list'].take(5).toList(); // only next 5 (next 15 hrs)
      });
    } else {
      setState(() {
 ...
 Widget buildHourlyItem(Map<String, dynamic> item) {
  ...
  final label = hour == DateTime.now().hour ? "Now" : "${hour % 12 == 0 ? 12 : hour % 12} ${hour >= 12 ? 'PM' : 'AM'}";
    ...
    children: [
      const Row(
        children: [
          ...
          SizedBox(width: 8),
          Text('Hourly Forecast', style: TextStyle(color: Colors.white, fontSize: 18)),
        ],
      ),
      const SizedBox(height: 10),
      SizedBox(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (context, index) {
            return buildHourlyItem(hourlyForecast![index]);
          },
        ),
      ),
    ],
